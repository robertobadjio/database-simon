...
func TestTCPClient(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"client with small max message size": {
			request: "hello server",
			client: func() *TCPClient {
				client, err := NewTCPClient(serverAddress, WithClientBufferSize(5))
				require.NoError(t, err)
				return client
			},
		...
		"client with idle timeout": {
			request: "hello server",
			client: func() *TCPClient {
				client, err := NewTCPClient(serverAddress, WithClientIdleTimeout(100*time.Millisecond))
				require.NoError(t, err)
				return client
			},
			expectedResponse: serverResponse,
		...
		"client without options": {
			request: "hello server",
			client: func() *TCPClient {
				client, err := NewTCPClient(serverAddress)
				require.NoError(t, err)
				return client
			},
			expectedResponse: serverResponse,

...
func TestSegmentWrite(t *testing.T) {
	...
	err := os.Mkdir(testWALDirectory, os.ModePerm) // nolint : G301: Expect directory permissions to be 0750 or less (gosec)
	require.NoError(t, err)

	defer func() {
		errRemoveAll := os.RemoveAll(testWALDirectory)
		require.NoError(t, errRemoveAll)
	}()
